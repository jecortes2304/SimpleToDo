...
import {ApiResponse, Pagination, SortOrder} from '../schemas/global'
import {AxiosResponse} from 'axios'
import {handleApiError, handleApiResponse} from '../utils/api-utils'
import {AdminUpdateUserRequestDto, AISettingsDto, UpdateAISettingsDto, UpdateUserRequestDto, UserResponseDto} from '../schemas/user'
...
export async function getAllUsers(
    ...
    return dedupeRequest(`users:${limit}:${page}:${sort}:${normalizedSearch}`, async () => {
        try {
            ...
            return handleApiError<Pagination<UserResponseDto>>(
                error as AxiosResponse<ApiResponse<Pagination<UserResponseDto>>>,
            );
        }
    });
}

export async function updateUser(id: number, data: AdminUpdateUserRequestDto): Promise<ApiResponse<UserResponseDto>> {
    try {
