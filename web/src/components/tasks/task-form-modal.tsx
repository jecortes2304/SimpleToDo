...
import {Task, TaskStatus} from "../../schemas/task.ts";
import {Project} from "../../schemas/project.ts";
import {Pagination, SortOrder} from "../../schemas/global.ts";
...
const ModalCreateUpdateTask: React.FC<ModalCreateTaskProps> = ({
                                                                   modalTitle,
                                                                   onCreate,
                                                                   onEdit,
                                                                   task,
    ...
    const [sort] = useState<SortOrder>('asc');
    ...
    const checkUserProjects = useCallback(async () => {
        ...
        const availableProjects = projectPagination.items
        const preferredProjectId = editMode && task ? task.projectId : initialProjectId

        setProjects(availableProjects)
        setProjectIdSelected(previousProjectId => {
            if (preferredProjectId && availableProjects.some(project => project.id === preferredProjectId)) {
                return preferredProjectId
            }
            if (availableProjects.some(project => project.id === previousProjectId)) {
                return previousProjectId
            }
            return availableProjects[0]?.id || 0
        })
    }, [editMode, initialProjectId, limit, page, sort, task])
    ...
    useEffect(() => {
        if (editMode && task) {
            ...
            setProjectIdSelected(task.projectId)
        }
    }, [editMode, task])
    ...
    return (
        <dialog id="modalCreateTask" className="modal">
            <div className="modal-box">
                <form method="dialog" onSubmit={(event) => handleSubmit(event)} className="flex flex-col">
                    ...
                    <div className="modal-action">
                        ...
                        <button type="submit" className={`btn btn-primary ${error && 'btn-disabled'}`}>
                            OK
                        </button>
                    </div>
                </form>
