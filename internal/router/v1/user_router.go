...
import (
	"net/http"
	request "simpletodo/internal/dto/request"
...
func (uc *UserController) updateUser(c echo.Context) error {
	...
	var body request.AdminUpdateUserRequest
	if err := bindAndValidate(c, &body); err != nil {
		return err
	}
	updatedUser, err := uc.UserService.UpdateByAdmin(uint(id), body)
	if err != nil {
...
func (uc *UserController) updateProfile(c echo.Context) error {
	...
	updatedUser, err := uc.UserService.UpdateProfile(uint(id), body)
	if err != nil {
		return response.WriteJSONResponse(c, http.StatusInternalServerError, "Failed to update", err.Error(), true)
	}
	return response.WriteJSONResponse(c, http.StatusOK, "User updated successfully", updatedUser, false)
...
func UserRouters(db *gorm.DB, v1 *echo.Group) {
	...
	aiServerRepository := repository2.NewAIServerRepository(db)
	...
	// @Security BearerAuth
	// @Param id path int true "User ID"
	// @Param user body request.AdminUpdateUserRequest true "Administrative user update payload (email is immutable)"
	// @Success 200 {object} response.UserResponse
	// @Failure 400 {object} response.StandardResponseError
	// @Failure 401 {object} response.StandardResponseError
	// @Failure 403 {object} response.StandardResponseError
	...
	// @Tags Profile
	// @Security BearerAuth
	// @Success 200 {object} response.AISettingsResponse
	// @Failure 401 {object} response.StandardResponseError
	// @Failure 500 {object} response.StandardResponseError
